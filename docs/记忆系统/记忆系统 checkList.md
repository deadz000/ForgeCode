...
- [ ] 包可正常导入：`python -c "import forgecode"`（验证：exit code 0）
- [ ] 所有单元测试通过：`pytest`（验证：exit code 0，无 FAIL）
- [ ] lint 无告警：`ruff check src/forgecode tests`（验证：exit code 0）
- [ ] 格式一致：`ruff format --check src/forgecode tests`（验证：exit code 0）
...
- [ ] Session ID 新格式：启动进程，检查 `.forgecode/sessions/` 下目录名形如 `YYYYMMDD-HHMMSS-xxxx`（验证：`ls .forgecode/sessions/`，正则匹配 `\d{8}-\d{6}-[0-9a-f]{4}`）
...
- [ ] 过期清理：手动创建时间戳为 31 天前的 session 目录 → 启动进程后被删除（验证：`ls .forgecode/sessions/` 不含该目录）
...
- [ ] 显式记忆触发：对话中说"记住 xxx" → Agent 回复后，memory 目录出现新 .md 文件（验证：`ls .forgecode/memory/` 或 `~/.forgecode/memory/`）
- [ ] 每 5 轮自动触发：连续对话 5 轮后检查 memory 目录是否有新增（验证：对比前后 `MEMORY.md`）
- [ ] 项目级分类：说"记住这个项目用中文" → 笔记出现在 `.forgecode/memory/`（项目级），type 为 `project_knowledge`（验证：`ls .forgecode/memory/`）
...
- [ ] 场景 9（过期清理）：手动创建 `.forgecode/sessions/20260401-120000-dead/conversation.jsonl`（31 天前的时间戳）和一个旧格式目录 `1717000000-abc12345/`；启动 → `ls .forgecode/sessions/` → 31 天前的目录被删，旧格式目录保留，当前新 session 目录存在。(AC19/AC20)
- [ ] 场景 10（记忆积累与注入）：启动 → 输入 "以后回复都用中文简体，不要用繁体" → 模型回复 → 等 2-3 秒（异步记忆更新）→ `ls .forgecode/memory/` 或 `~/.forgecode/memory/` 出现 .md 笔记文件；`cat */MEMORY.md` 有对应摘要行。退出 → 重新启动新会话 → 输入 "你好" → 确认模型回复为简体中文（记忆生效）。(AC21/AC22/AC23)
- [ ] 场景 11（运行中不可 resume）：启动 → 输入一个需要较长时间的任务（如 "读取所有 Python 文件"）→ 在模型思考/工具执行期间输入 `/resume` → TUI 显示"请等待当前任务完成"提示，不进入列表。(AC29)
- [ ] 场景 12（压缩后 JSONL 标记）：配置一个小 `context_window`（如 2000）→ 对话数轮触发自动压缩 → `grep '"type":"compact"' .forgecode/sessions/*/conversation.jsonl` 至少命中一次；`/exit` → 重启 → `/resume` 恢复该会话 → 消息数远小于 JSONL 总行数（从 compact 标记后加载）。(AC9/AC16)