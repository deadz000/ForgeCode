...
> 包名：`forgecode`（Python 3.12+）。源码位于 `src/forgecode/`。

## 文件清单

| 操作 | 文件 | 职责 |
|------|------|------|
| 修改 | `src/forgecode/compact/state.py` | session ID 格式变更、`SessionContext` 加 `session_dir`、`open_session_context` |
| 修改 | `src/forgecode/conversation.py` | `from_messages` 类方法、回调触发 |
| 修改 | `tests/test_conversation.py` | 回调测试 |
| 修改 | `src/forgecode/prompt.py` | `build_system_prompt` 签名变更 |
| 修改 | `src/forgecode/prompt_modules.py` | `optional_modules` 改为接受参数 |
| 修改 | `tests/test_prompt.py` | 新签名测试 |
| 新建 | `src/forgecode/instructions/__init__.py` | 子包标识 |
| 新建 | `src/forgecode/instructions/loader.py` | Loader 类、三层加载、@include 展开 |
| 新建 | `tests/test_instructions_loader.py` | @include 深度/环路/逃逸/缺失文件测试 |
| 新建 | `src/forgecode/session/__init__.py` | 子包标识 |
| 新建 | `src/forgecode/session/writer.py` | Writer、Entry、append、`open_existing` |
| 新建 | `src/forgecode/session/list.py` | `list_sessions`、SessionInfo |
| 新建 | `src/forgecode/session/load.py` | `load_session`、坏行跳过、孤立截断 |
| 新建 | `src/forgecode/session/cleanup.py` | `clean_expired`、ID 时间戳解析 |
| 新建 | `tests/test_session.py` | JSONL 读写、列表、恢复、清理测试 |
| 新建 | `src/forgecode/memory/__init__.py` | 子包标识 |
| 新建 | `src/forgecode/memory/types.py` | NoteType、Note、UpdateAction |
| 新建 | `src/forgecode/memory/store.py` | Store、笔记文件 CRUD、索引读写 |
| 新建 | `src/forgecode/memory/manager.py` | Manager、`load_index`、`update_async` |
| 新建 | `src/forgecode/memory/prompts.py` | 记忆更新 prompt 模板 |
| 新建 | `tests/test_memory.py` | 索引加载、操作执行、截断测试 |
| 修改 | `src/forgecode/agent/agent.py` | run 末尾触发记忆更新 |
| 修改 | `src/forgecode/agent/runtime.py` | 接受 `memory_manager` 注入 |
| 修改 | `src/forgecode/tui/commands.py` | /resume 命令注册 |
| 新建 | `src/forgecode/tui/resume.py` | `RESUMING` 状态、会话列表项、`handle_resume` |
| 修改 | `src/forgecode/tui/app.py` | `RESUMING` 状态集成、App 新增字段 |
| 修改 | `src/forgecode/cli.py` | 启动流程串联 |
| 修改 | `.forgecode/config.yaml.example` | 配置示例补充说明 |

## T1: Session ID 格式变更**文件：** `src/forgecode/compact/state.py`
**依赖：** 无
**步骤：**
...
2. `SessionContext` 新增 `session_dir: str` 字段，值为 `<workspace>/.forgecode/sessions/<session_id>`
...
## T2: Conversation 回调机制**文件：** `src/forgecode/conversation.py`、`tests/test_conversation.py`
**依赖：** 无
**步骤：**
...
## T3: 项目指令加载器**文件：** `src/forgecode/instructions/__init__.py`、`src/forgecode/instructions/loader.py`、`tests/test_instructions_loader.py`
**依赖：** 无
**步骤：**
...
## T4: Session Writer**文件：** `src/forgecode/session/__init__.py`、`src/forgecode/session/writer.py`
...
**验证：** `python -c "from forgecode.session.writer import Writer"` 不报错

## T5: 会话列表扫描**文件：** `src/forgecode/session/list.py`
...
**验证：** `python -c "from forgecode.session.list import list_sessions"` 不报错

## T6: 会话加载恢复**文件：** `src/forgecode/session/load.py`
**依赖：** T4（Entry 类型）
**步骤：**
...
**验证：** `python -c "from forgecode.session.load import load_session"` 不报错

## T7: 会话过期清理**文件：** `src/forgecode/session/cleanup.py`
**依赖：** T1（`parse_session_time`）
**步骤：**
...
**验证：** `python -c "from forgecode.session.cleanup import clean_expired"` 不报错
...
## T9: 笔记类型与存储**文件：** `src/forgecode/memory/__init__.py`、`src/forgecode/memory/types.py`、`src/forgecode/memory/store.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from forgecode.memory.store import Store"` 不报错

## T10: 记忆管理器**文件：** `src/forgecode/memory/manager.py`、`src/forgecode/memory/prompts.py`
**依赖：** T9
**步骤：**
...
**验证：** `python -c "from forgecode.memory.manager import Manager"` 不报错
...
## T12: build_system_prompt 参数化**文件：** `src/forgecode/prompt.py`、`src/forgecode/prompt_modules.py`、`tests/test_prompt.py`
**依赖：** 无
**步骤：**
...
## T13: /resume 命令注册**文件：** `src/forgecode/tui/commands.py`
**依赖：** 无
**步骤：**
...
**验证：** `python -c "from forgecode.tui.commands import BUILTIN_COMMANDS"` 不报错

## T14: 会话列表 UI**文件：** `src/forgecode/tui/resume.py`、`src/forgecode/tui/app.py`
...
**验证：** `python -c "from forgecode.tui.resume import begin_resume"` 不报错

## T15: Agent 记忆更新触发**文件：** `src/forgecode/agent/agent.py`、`src/forgecode/agent/runtime.py`
**依赖：** T10（Manager）
**步骤：**
...
## T16: cli.py 启动流程串联**文件：** `src/forgecode/cli.py`
