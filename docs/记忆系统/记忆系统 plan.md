...
| 已有模块 | 改动 |
|--------|------|
| `forgecode.prompt` | `build_system_prompt` 接受 instructions/memory 参数 |
| `forgecode.conversation` | 新增 on_append/on_replace 回调 |
| `forgecode.compact.state` | session ID 格式改为 YYYYMMDD-HHMMSS-xxxx；`SessionContext` 加 `session_dir` 字段 |
| `forgecode.agent` | 每 5 轮 run 结束后触发记忆更新 |
| `forgecode.tui` | 新增 /resume 命令和 `RESUMING` 状态 |
| `forgecode.cli` | 启动流程串联指令加载、记忆初始化、会话清理 |
